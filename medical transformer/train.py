...
import timeit
import sys
sys.path.insert(1, '/users/zlyu12/Desktop/cs1430/CSCI1430-Final-Project-MedImage-Segmentation')
...
parser.add_argument('--imgsize', type=int, default=240)
...
dataset = NiiDataset("/users/zlyu12/Desktop/cs1430/data/MICCAI_BraTS_2019_Data_Training/HGG")
...
for epoch in range(args.epochs):
    ...
    for batch_idx, (X_batch, y_batch) in enumerate(dataloader):        
        ...
        X_batch = Variable(X_batch.to(device ='cuda', dtype=torch.float))
        y_batch = Variable(y_batch.to(device='cuda', dtype=torch.long))
    ...
    if (epoch % args.save_freq) ==0:
        ...
        for batch_idx, (X_batch, y_batch) in enumerate(valloader):
            ...
            X_batch = Variable(X_batch.to(device='cuda', dtype=torch.float))
            y_batch = Variable(y_batch.to(device='cuda', dtype=torch.float))
